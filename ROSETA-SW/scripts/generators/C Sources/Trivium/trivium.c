...
/* Generate a keystream with length bytes * 4, each one is printed in hexa format
** on a text file indicated by "sdout" parameter, the file name is informed by
** user by the '>' operator in prompt.
*/
...
#define Z(w) (z = w)
  ...
  for(i = 0; i < length; i++)
  {
    ...
    // Print the keystream byte in the text file (stdout).

    //putc(z, stdout);
    
    zTEMP = SWAP32(z);

    b0 = reverse(zTEMP & 0xFF);
    b1 = reverse(zTEMP>>8 & 0xFF);
    b2 = reverse(zTEMP>>16 & 0xFF);
    b3 = reverse(zTEMP>>24 & 0xFF);

    res = b0 << 24 | b1 << 16 | b2 << 8 | b3;

    fprintf(stdout,"%08X", res);     // Reverse Order - 64FD07DF
    //fprintf(stdout,"%08X", z);           // Normal Order  - DF07FD64
